...
func consumeIdent(s string) *tokenize.Token {
	if token.Kind == tokenize.Ident && s == token.Lit.GetString() {
		tok := token
		token = token.Next
		return tok
	}
...
func toplevel() (*Node, error) {
	...
	if t := consumeKind(tokenize.Comment); t != nil {
		return NewCommentNode(t.Pos, t.Lit.GetString()), nil
	}
	...
	if t := consumeIdent("func"); t != nil {
		...
		return NewFuncDefNode(t.Pos,
			NewIdentNode(id.Pos, id.Lit.GetString()),
			params,
			returns,
			body), nil
	}
	...
	if t := consumeIdent("import"); t != nil {
		...
		return NewImportNode(t.Pos, target.Lit.GetString()), nil
	}
	...
	if c := consumeIdent("var"); c != nil {
		...
		if a := consumeKind(tokenize.Assign); a == nil {
			// var decl
			return NewVarDeclNode(c.Pos, NewIdentNode(id.Pos, id.Lit.GetString()), typ), nil
		}
		// var assign
		...
		return NewAssignNode(c.Pos, NewVarDeclNode(c.Pos, NewIdentNode(id.Pos, id.Lit.GetString()), typ), value), nil
	}
...
func stmt() (*Node, error) {
	...
	if comment := consumeKind(tokenize.Comment); comment != nil {
		return NewCommentNode(comment.Pos, comment.Lit.GetString()), nil
	}
	...
	if return_ := consumeIdent("return"); return_ != nil {
		...
		return NewPolynomialNode(Return, return_.Pos, values), nil
	}
...
func assign() (*Node, error) {
	...
	if var_ := consumeIdent("var"); var_ != nil {
		...
		idNode := NewIdentNode(id.Pos, id.Lit.GetString())
		// イコール、代入がなかった場合
...
func andor() (*Node, error) {
	...
	for {
		if and := consumeKind(tokenize.And); and != nil {
			...
			n = NewBinaryNode(And, and.Pos, n, rhs)
		} else if or := consumeKind(tokenize.Or); or != nil {
			rhs, err := equality()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Or, or.Pos, n, rhs)
		} else {
			break
		}
	}
...
func equality() (*Node, error) {
	...
	for {
		if eq := consumeKind(tokenize.Eq); eq != nil {
			...
			n = NewBinaryNode(Eq, eq.Pos, n, rhs)
		} else if ne := consumeKind(tokenize.Ne); ne != nil {
			rhs, err := relational()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Ne, ne.Pos, n, rhs)
		} else {
			break
		}
	}
...
func relational() (*Node, error) {
	...
	for {
		if lt := consumeKind(tokenize.Lt); lt != nil {
			...
			n = NewBinaryNode(Lt, lt.Pos, n, rhs)
		} else if le := consumeKind(tokenize.Le); le != nil {
			rhs, err := add()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Le, le.Pos, n, rhs)
		} else if gt := consumeKind(tokenize.Gt); gt != nil {
			rhs, err := add()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Gt, gt.Pos, n, rhs)
		} else if ge := consumeKind(tokenize.Ge); ge != nil {
			rhs, err := add()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Ge, ge.Pos, n, rhs)
		} else {
			break
		}
	}
...
func add() (*Node, error) {
	...
	for {
		if plus := consumeKind(tokenize.Add); plus != nil {
			...
			n = NewBinaryNode(Add, plus.Pos, n, rhs)
		} else if minus := consumeKind(tokenize.Sub); minus != nil {
			rhs, err := mul()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Sub, minus.Pos, n, rhs)
		} else {
			break
		}
	}
...
func mul() (*Node, error) {
	...
	for {
		if star := consumeKind(tokenize.Mul); star != nil {
			...
			n = NewBinaryNode(Mul, star.Pos, n, rhs)
		} else if div := consumeKind(tokenize.Div); div != nil {
			rhs, err := unary()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Div, div.Pos, n, rhs)
		} else if mod := consumeKind(tokenize.Mod); mod != nil {
			rhs, err := unary()
			if err != nil {
				return nil, err
			}
			n = NewBinaryNode(Mod, mod.Pos, n, rhs)
		} else {
			break
		}
	}
...
func unary() (*Node, error) {
	if plus := consumeKind(tokenize.Add); plus != nil {
		...
		return NewBinaryNode(
			Sub,
			minus.Pos,
			NewLiteralNode(minus.Pos, core.NewLiteral(0)),
			v), nil
		...
		return NewUnaryNode(Not, not.Pos, v), nil
	}
	return primary()
}
...
func access() (*Node, error) {
	if peekKind(tokenize.Ident) != nil && peekNextKind(tokenize.Dot) != nil {
		...
		return NewPrefixNode(p.Pos, p.Lit.GetString(), l), nil
	}
	return literal()
}
...
func literal() (*Node, error) {
	...
	if lrb := consumeKind(tokenize.Lrb); lrb != nil {
		...
		return NewUnaryNode(Parenthesis, expression.Pos, expression), nil
	}
	...
	if id := consumeKind(tokenize.Ident); id != nil {
		...
		if lrb := consumeKind(tokenize.Lrb); lrb != nil {
			if consumeKind(tokenize.Rrb) != nil {
				return NewCallNode(id.Pos, NewIdentNode(id.Pos, id.Lit.GetString()), nil), nil
			}
			args, err := callArgs()
			if err != nil {
				return nil, err
			...
			return NewCallNode(id.Pos, NewIdentNode(id.Pos, id.Lit.GetString()), args), nil
		}
		// ident
		return NewIdentNode(id.Pos, id.Lit.GetString()), nil
	}

	if i := consumeKind(tokenize.Int); i != nil {
		return NewLiteralNode(i.Pos, i.Lit), nil
	}
	if f := consumeKind(tokenize.Float); f != nil {
		return NewLiteralNode(f.Pos, f.Lit), nil
	}
	if s := consumeKind(tokenize.String); s != nil {
		return NewLiteralNode(s.Pos, s.Lit), nil
	}
	if b := consumeKind(tokenize.Bool); b != nil {
		return NewLiteralNode(b.Pos, b.Lit), nil
	}
	if n := consumeKind(tokenize.Nil); n != nil {
		return NewLiteralNode(n.Pos, n.Lit), nil
	}
...
func types() (*Node, error) {
	...
	typ, err := core.GetDataTypeByIdent(id.Lit.GetString())
	if err != nil {
		return nil, err
	}
	return NewDataTypeNode(id.Pos, typ), nil
}
...
func callArgs() (*Node, error) {
	...
	return NewPolynomialNode(Args, args[0].Pos, args), nil
}
...
func funcParams() (*Node, error) {
	...
	firstId := NewIdentNode(firstIdT.Pos, firstIdT.Lit.GetString())
	firstType, err := types()
	if err != nil {
		return nil, err
	}
	...
	for consumeKind(tokenize.Comma) != nil {
		...
		id := NewIdentNode(idt.Pos, idt.Lit.GetString())
		typ, err := types()
		if err != nil {
			return nil, err
		}
	...
	return NewPolynomialNode(Params, firstIdT.Pos, params), nil
}
...
func funcReturns() (*Node, error) {
	...
	if lrb == nil {
		...
		return NewPolynomialNode(ReturnTypes, typ.Pos, []*Node{typ}), nil
	}
	...
	return NewPolynomialNode(ReturnTypes, lrb.Pos, returnTypes), nil
}
